//! Implementation for decompressing the DEFLATE format based on RFC 1951.
//!
//! The only user-facing module here is the [Inflate](struct.Inflate.html) type.
//! You can decompress raw DEFLATE-compressed files with it.
//!
//! # Basic usage
//!
//! Just make sure to have `std::io::Read` in scope:
//!
//! ```
//! use std::io::Read;
//! use cacti_archive::deflate::Inflate;
//!
//! // The string "abcabcabcabcabcabcabcabcabcabc" DEFLATEd
//! const DATA: &[u8] = &[
//!     0x4a, 0x4c, 0x4a, 0x4e, 0xc4, 0x8d, 0x00,
//!     0x00, 0x00, 0x00, 0xff, 0xff, 0x03, 0x00];
//! let mut result = Vec::new();
//! let mut inflate = Inflate::new(DATA);
//! inflate.read_to_end(&mut result);
//! assert_eq!(result, "abcabcabcabcabcabcabcabcabcabc".bytes().collect::<Vec<_>>());
//! ```
...
mod tests {
    ...
    #[test]
    fn test_empty() {
        const DATA: [u8; 7] = [0x00, 0x00, 0x00, 0xff, 0xff, 0x03, 0x00];
        assert_eq!(inflate(&DATA), &[]);
    }

    #[test]
    fn test_hello_world() {
        const DATA: [u8; 21] = [
            0xf2, 0x48, 0xcd, 0xc9, 0xc9, 0xd7, 0x51, 0x08, 0xcf, 0x2f,
            0xca, 0x49, 0x51, 0x04, 0x00, 0x00, 0x00, 0xff, 0xff, 0x03, 0x00];
        assert_eq!(inflate(&DATA), "Hello, World!".bytes().collect::<Vec<_>>());
    }

    #[test]
    fn test_hello_world_times_1024() {
        const DATA: [u8; 117] = [
            0xec, 0xd5, 0x31, 0x0d, 0x00, 0x20, 0x10, 0x03, 0x40, 0x2b, 0xb0,
            0x23, 0x04, 0x07, 0x18, 0x80, 0xad, 0xc9, 0x27, 0xf8, 0x1f, 0xb0,
            0xc1, 0x70, 0xe3, 0xaf, 0xcd, 0xb7, 0x37, 0x4f, 0x52, 0xa3, 0xad,
            0xba, 0xd9, 0x7d, 0x3a, 0x64, 0xe0, 0x0f, 0x74, 0xc1, 0x1e, 0xd8,
            0x44, 0x2e, 0xe0, 0x10, 0x87, 0x38, 0xc4, 0x21, 0x0e, 0x71, 0x88,
            0x43, 0x1c, 0xe2, 0x10, 0x87, 0x38, 0xc4, 0x21, 0x0e, 0x71, 0x88,
            0x43, 0x1c, 0xe2, 0x10, 0x87, 0x38, 0xc4, 0x21, 0x0e, 0x71, 0x88,
            0x43, 0x1c, 0xe2, 0x10, 0x87, 0x38, 0xc4, 0x21, 0x0e, 0x71, 0x88,
            0x43, 0x1c, 0xe2, 0x10, 0x87, 0x38, 0xc4, 0x21, 0x0e, 0x71, 0x88,
            0x43, 0x1c, 0xe2, 0x10, 0x87, 0x38, 0xc4, 0x61, 0x3e, 0xca, 0xe0,
            0x01, 0x00, 0x00, 0xff, 0xff, 0x03, 0x00];
        let mut hello = String::new();
        for _ in 0..1024 {
            hello += "Hello, World!";
        }
        assert_eq!(inflate(&DATA), hello.bytes().collect::<Vec<_>>());
    }
}
